...
export default function Navbar() {
  ...
  // Close menu on click outside menu or button
  useEffect(() => {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full z-50 h-16 md:h-20 bg-white shadow-md cursor-pointer">
        ...
        <div className="flex items-center justify-between h-full">
          ...
          <Link href="/" className="flex items-center h-full space-x-2">
            <div className="relative h-10 w-10 sm:h-12 sm:w-12 md:h-14 md:w-14 lg:h-16 lg:w-16">
              <Image src="/logo_2.png" alt="InterIIT Logo" fill className="object-contain" priority />
            </div>
          ...
          {/* Desktop navigation */}
          <div className="hidden md:flex items-center space-x-8" style={{ fontFamily: "'Mukta',sans-serif" }}>
            {navItems.map((item) => {
          ...
          <div className="md:hidden flex items-center" ref={buttonRef}>
            ...
            >
              {!isOpen ? (
                <svg className="h-6 w-6 text-black cursor-pointer font-bold" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                  <path strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" d="M4 6h16M4 12h16M4 18h16" />
                </svg>
              ) : (
                <svg className="h-6 w-6 text-[#800000] cursor-pointer font-bold" viewBox="0 0 24 24" fill="none" stroke="currentColor">
      ...
      >
        <div className="flex flex-col items-start px-6 pt-6 space-y-2 cursor-pointer">
          {navItems.map((item) => {
            const isActive = pathname === item.href;
            return (
              <Link
